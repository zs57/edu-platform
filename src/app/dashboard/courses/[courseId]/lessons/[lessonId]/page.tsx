...
export default async function LessonPage({ params }: { params: Promise<{ courseId: string; lessonId: string }> }) {
  ...
  // Fetch all completed lessons for THIS chapter's sidebar to show markers if we want
  ...
  return (
    <div className="max-w-6xl mx-auto pb-12 relative z-10" dir="rtl">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 border-b border-white/5 pb-8 gap-4">
        <div>
          <Link href={`/dashboard/courses/${resolvedParams.courseId}`} className="text-zinc-500 hover:text-white flex items-center gap-2 mb-4 transition-colors text-sm font-black group w-fit">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="space-y-6">
          <div className="panel p-8 sticky top-24 border-blue-500/20 shadow-2xl bg-zinc-900/40 backdrop-blur-xl group overflow-hidden">
            ...
            {lesson.quizzes.length > 0 ? (
              <Link href={`/dashboard/quiz/${lesson.quizzes[0].id}`} className="w-full btn-primary text-xl py-5 mb-8 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 shadow-xl shadow-amber-900/20 flex items-center justify-center gap-3 border-none group">
                <HelpCircle className="w-6 h-6 group-hover:rotate-12 transition-transform" />
                امتحان الدرس المباشر
              </Link>
            ) : (
              <div className="w-full text-center p-6 rounded-2xl border border-white/5 bg-black/40 text-zinc-600 font-black text-sm mb-8">
            ...
            <div className="pt-8 border-t border-white/5">
              <div className="flex items-center justify-between mb-6">
                 <h4 className="text-xs font-black text-zinc-500 uppercase tracking-[0.2em]">فهرس الدروس المجاورة</h4>
                 <span className="text-[10px] font-black bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded-md">مفتوح للجميع</span>
              </div>
                  ...
                  return (
                    <Link 
                      ...
                      className={`flex items-center gap-3 p-4 rounded-2xl transition-all border ${
                        isCurrent 
                          ? 'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-500/20 active-scale' 
                          : 'bg-white/5 border-white/5 text-zinc-400 hover:border-white/10 hover:text-white hover:bg-white/[0.08]'
                      }`}
                    >
